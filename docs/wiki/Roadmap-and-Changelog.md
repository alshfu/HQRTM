...
- **2026-06-10** — **Vitrinen visar våning/balkong/kök**: `gen_sample_listings.py` aggregerar nu
  **HomeQ + Qasa (Göteborg)** och behåller `floor/has_balcony/has_kitchen` (Qasas rika fritext ger
  riktiga bekvämlighetsfält via `extract_features`). Modulära demot (kort + modal) och skyltfönstret
  (`index.html`) visar fälten. Ingen fiktion. Device-snapshots är fortsatt frysta referenser.
